...
rownames(d.mat) <- colnames(d.mat) <- paste(colnames(mat),colData$Flowcell, colData$Sex, colData$Group,sep = " : ")
hc <- hclust(distsRL)
hmcol <- colorRampPalette(brewer.pal(9, "GnBu"))(100)
pdf("top1000_variable_genes.distance_plot.pdf")
...
pheatmap(z.mat[names(sort(km.z.mat$cluster)),],
         ...
         show_rownames = F,
         show_colnames = T,
         filename = "top1000_variable_genes.kmeans_heatmap.pdf"
)

#2D PCA plot
mat.pca <- as.data.frame(prcomp(t(assay(vst)))$x)
mat.pca$Group <- colData$Group #Add group metadata
mat.pca$Flowcell <- colData$Flowcell #Add flowcell metada
mat.pca$Sex <- colData$Sex #Add flowcell metada
...
ggsave("top1000_variable_genes.PCA_plot_Group.pdf")
...
ggsave("top1000_variable_genes.PCA_plot_Sex.pdf")
...
ggsave("top1000_variable_genes.PCA_plot_Flowcell.pdf")
